%Create a document to pass to graphviz to visualize merging with m.
%This version tries to merge at endpoint instead of intermeidate pt
%and uses the sticky files as input. 
clear
...
for i=49:-1:1
    %Read data from file
    filename=strcat('n',num2str(n),'m',num2str(i),'Sticky',Eout(test),'TEST.txt'); %File name nxrhoy.txt
    ...
    for j=1:num_clusters
        ...
        if mergeYet(j)==1
            ...
            if length(mergeList)>1
                ...
                fprintf(fileID,'"%dp%d" [label="%d \\n m=%d"  ,  shape=circle  , regular=1,style=filled,fillcolor=white, width=0.1   ] ;\n',j,i,j,i);
                fprintf(fileID, '{"%dp%d"',j,mergeIndex(j));
