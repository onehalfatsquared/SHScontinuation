%Perform continuation on a cluster that re-arranges when it merges. Store
%the pre-merge cluster and post merge cluster and compute the rmsd
clear
...
rho=50; rho_end=1;  draw=0; eigTol=-1e-6; Etol=1e-14; %Define parameters
...
%Perform rho descent.  % rho = 6.7
clus = 8; %cluster to test
rc = 0; rp = 0; %current and previous rmin values to detect jump
cc = zeros(1,3*n); cp = zeros(1,3*n); %current and previous cluster
tol = 0.02; 
for alpha=0:0.1:rho-rho_end
    ...
    di=unique(pDists(c2p(cluster))); di=di(di>1.02); %use 1.01 for rho 38, 1.05 for rho 6
    rc = min(di)
    cc = cluster; 
    if alpha == 0
        rp=rc;
    end
    %Test if jump occured
    if abs(rp-rc)>tol
        break
    else
        rp = rc;
