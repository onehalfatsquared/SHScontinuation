...
m=50; m_end=1; record=1; eigTol=-1e-7; Etol=1e-14; %Define parameters
...
%Open the matlabpool for parallel workers
parpool('local',min(num_clusters-1,4));

%Perform rho descent. 
for alpha=0:0.1:m-m_end
    %Determine the energy  for this rho value to keep kappa const.
    E=stickyNewton(E, m-alpha, kap);
    parfor i=1:num_clusters %num_clusters
    ...
    if rem(m-alpha+1,1)<1e-2 && record==1
        %rho is an integer, write clusters to a file
        m-alpha %print to show progress
        filename=strcat('n',num2str(n),'m',num2str(round(m-alpha)),'Sticky',Eout(test),'.txt'); %File name nxrhoy.txt
...
%Close the parallel worker pool
delete(gcp('nocreate'));
