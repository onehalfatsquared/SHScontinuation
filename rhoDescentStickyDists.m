...
%every integer value of rho. Keep constant sticky parameter. Keeps track of
%smallest particle distance >1 and plots
...
rho=50; rho_end=1; record=0; eigTol=-1e-6; Etol=1e-14; %Define parameters
...
for alpha=0:0.1:rho-rho_end
    ...
    q(count)=pDist(cluster);
    count=count+1;
    %Write new clusters to file
    if rem(rho-alpha+1,1)<1e-2 && record==1
        %rho is an integer, write clusters to a file
        rho-alpha %print to show progress
        filename=strcat('n',num2str(n),'rho',num2str(round(rho-alpha)),'Sticky',Eout(test),'.txt'); %File name nxrhoy.txt
...
q=q(end:-1:1);
plot(x,q)
xlabel('rho'); ylabel('Smallest Distance');
title(['n=' num2str(n) ' Cluster ' num2str(i) ' Smallest Distance >1 plot'])
...
function minDist=pDist(cluster)
%determine min distance>1 for cluster
minDist=10;
n=length(cluster)/3;
for i=1:n
    for j=i+1:n
        d=euDist(c2p(cluster),i,j);
        if d<minDist
            minDist=d;
        end
    end
