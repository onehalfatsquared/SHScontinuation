%Scale down m from 50 to 1 in steps of alpha (0.1) and record the clusters
%every integer value of m (LJ potential). Keep constant sticky parameter. 
%clear
...
m=50; m_end=1; draw=0; eigTol=-1e-7; Etol=1e-14; %Define parameters
...
kap=stickyEval(E,m); %Evaluate the initial sticky parameter

%Perform rho descent. 
count=1; last=0; %lastD=pDists(clusters(1,:)); 
for alpha=0:0.1:m-m_end
    %Determine the energy  for this rho value to keep kappa const.
    E=stickyNewton(E, m-alpha, kap);
    for i=2:2 %num_clusters
        ...
        [cluster,c]=conjGradDesc(cluster,m-alpha,E);
        if errorCheck(cluster,err_catch,m-alpha,E)==0
            %If CG doesnt work, reset cluster and call the errorDescent 
            cluster=err_catch; 
            [cluster,f]=errorDescent(cluster,m-alpha,E); 
            if f==1
        ...
        eigvals=eig(hessLJ(cluster,m-alpha,E));
        negs=length(find(eigvals<eigTol)); 
        if negs>0
            fprintf('Cluster %d has %d negative eigenvalue(s) at rho=%.2f.\n',i,negs,m-alpha)
            norm(LJgrad(cluster,m-alpha,E))
            cluster=escapeSaddle(cluster,m-alpha,E);
            if norm(cluster)>10
                cluster=err_catch;
            end
        end
    ...
    %q(count)=min(eigvals(eigvals~=0));
    di=unique(pDists(c2p(cluster))); di=di(di>1.01);
    q(count)=min(di);
    count=count+1;
    %Write new clusters to file
    if rem(m-alpha+1,1)<1e-2 && draw==1
        %rho is an integer, write clusters to a file
        m-alpha %print to show progress
...
x=linspace(1,50,length(q));
q=q(end:-1:1);
plot(x,q,'linewidth',2)
%xlabel('\rho'); ylabel('Minimum Eigenvalue');
%title(['N=' num2str(n) ' Cluster ' num2str(i) ' Minimum Eigenvalue Plot'])
xlabel('\rho'); ylabel('r_{min}');
%title(['N=' num2str(n) ' Cluster ' num2str(i) ', Plot of r_{min} vs. m'])
...
function [cluster, counter]=conjGradDesc(cluster, rho, E)
    ...
    g0=-LJgrad(cluster,rho,E); %Gradient of potential
    ...
    while counter<cutoff
        ...
        gsig=LJgrad(cluster+sigma*h,rho,E);
...
function [clust_out, counter]=conjGradDescRes(cluster, rho, E)
    ...
    while counter<cutoff
        ...
        e2=LJ(c2p(cluster),rho,E); ediff=e2-e1; e1=e2;
        if ediff>eTol
            return
        end
    end
...
function [cluster,particles,counter]=gradDesc(cluster, particles, rho, E)
    ...
    alpha=0.85;                       %Parameter to exp. for step
        for i=0:100
            L=LJ(c2p(cluster-alpha^i*g0),rho,E);
            R=LJ(particles,rho,E);
            if L<=R
    ...
    counter=1;
    while counter<cutoff
        g1=LJgrad(cluster1,rho,E); %Gradient of potential 
...
function left=escapeSaddle(clust, rho,E)
    ...
    H=hessLJ(clust,rho,E); %Hessian matrix
    ...
    while step<1
        ...
        Hleft=hessLJ(left,rho,E); eleft=eig(Hleft);
    ...
    while step<1
        ...
        Hright=hessLJ(right,rho,E); eright=eig(Hright);
...
function ok=errorCheck(clust, orig, rho,E)
    ...
    Einit=LJ(c2p(orig),rho,E); Efinal= LJ(c2p(clust),rho,E); %Initial and final energy
