...
m=50; m_end=1; record=0; eigTol=-1e-7; Etol=1e-14; %Define parameters
...
for alpha=0:step:m-m_end
    ...
    if rem(m-alpha+1,1)<1e-4 && record==1
        %rho is an integer, write clusters to a file
        m-alpha %print to show progress
        filename=strcat('n',num2str(n),'m',num2str(round(m-alpha)),'Sticky',Eout(test),'TEST2.txt'); %File name nxrhoy.txt
