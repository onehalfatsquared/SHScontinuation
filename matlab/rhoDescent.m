...
%Perform the rho descent
rho=50; E=0.3; rho_end=1; record=1; eigTol=-1e-7; Etol=1e-14; 
for alpha=0:0.1:rho-rho_end
    for i=1:num_clusters %num_clusters
    ...
    if rem(rho-alpha+1,1)<1e-2 && record==1
        ...
        filename=strcat('n',num2str(n),'rho',num2str(round(rho-alpha)),'TEST.txt'); %File name nxrhoy.txt
