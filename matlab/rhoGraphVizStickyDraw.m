...
%and uses the sticky files as input. Draws the clusters as images and
%displays them as nodes. 
clear
...
for i=49:-1:1
    ...
    if i==49
        ...
        filename=strcat('n',num2str(n),'rhoGraphSticky',Eout(test),'IMG.txt'); %File name nxrhoGraph.txt
        fileID=fopen(filename,'w');                     %Open file
        fprintf(fileID, 'digraph Merging_Diagram_n%d  {\n nodesep=0.1;\n',n);
        for j=1:num_clusters
            fprintf(fileID,'"%dp%d" [label="\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n%d \\n&#961;=%d"  , fontsize=18,  shape=none, width=3, height=3, fixedsize=true, scale=true, image="n%dc%dp%d.png"] ;\n',j,mergeIndex(j),j,mergeIndex(j),n,j,50);      %Write cluster
            %prettyCluster(clusters(j,:),n,j,50);
        end
    ...
    for j=1:num_clusters
        ...
        if mergeYet(j)==1
            ...
            if length(mergeList)>1
                ...
                %prettyCluster(clusters(j,:),n,j,i);
                filename=strcat('n',num2str(n),'rhoGraphSticky',Eout(test),'IMG.txt'); %File name nxrhoGraph.txt
                fileID=fopen(filename,'a+');                     %Open file
                fprintf(fileID,'"%dp%d" [label="\\n\\n\\n\\n\\n\\n%d \\n&#961;=%d"  , fontsize=18, shape=none, width=2.5, height=2.5, fixedsize=true, scale=true, image="n%dc%dp%d.png"] ;\n',j,i,j,i,n,j,i);
                fprintf(fileID, '{"%dp%d"',j,mergeIndex(j));
