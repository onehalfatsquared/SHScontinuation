%Scale down rho from 50 to 1 in steps of alpha (0.1) and compute the rmsd
%of two clusters as they undergo descent. 
clear
...
S = [1,2]; %set of clusters to compare
...
for alpha=0:0.1:rho-rho_end
    ...
    %compute rmsd of clusters
    [~,~,~,~,~,d] = testSame(clusters(S(1),:), clusters(S(2),:),1);
    q(count) = d;
    count=count+1;
end
