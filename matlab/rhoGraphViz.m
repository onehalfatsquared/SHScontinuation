    ...
    for j=1:num_clusters
        ...
        if mergeYet(j)==1
            ...
            if length(mergeList)>1
                ...
                fprintf(fileID, 'merge%d [shape=point, width=0.01, height=0.01];\n',merge);
                fprintf(fileID, '{"%dp%d"',j,mergeIndex(j));
                for nodes=mergeList(2:end)
                    fprintf(fileID, ', "%dp%d"',nodes,mergeIndex(nodes));
                end
                fprintf(fileID, '} -> merge%d [dir=none];\n', merge);
                mergeIndex(j)=i;
                fprintf(fileID,'"%dp%d" [label="%d \\n&#961;=%d"  ,  shape=circle  , regular=1,style=filled,fillcolor=white   ] ;\n',j,mergeIndex(j),j,mergeIndex(j));
                fprintf(fileID, 'merge%d -> "%dp%d" ;\n',merge, j, mergeIndex(j));
                merge=merge+1;
            end
        end
    end
